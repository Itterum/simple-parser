...
export default class GithubExtractor extends BaseExtractor<RepositoryEntity> {
  ...
  async parseEntity(element: ElementHandle): Promise<RepositoryEntity> {
    ...
    return new RepositoryEntity({
      title:
        (await titleElement?.textContent())?.trim().replace(/\s+/g, ' ') || '',
      url: new URL(
      ...
      countAllStars: parseInt(
        (await countAllStarsElement?.textContent())?.trim().replace(',', '') ||
          '0',
      ),
      countStarsToday: parseInt(
        (await countStarsTodayElement?.textContent())
          ?.trim()
          .replace(',', '') || '0',
      ),
      countForks: parseInt(
        (await countForksElement?.textContent())?.trim().replace(',', '') ||
          '0',
      ),
    });
  }
  ...
  async setupPage(
    page: Page,
    blockedResources: string[] = ['png', 'jpeg', 'jpg', 'svg', 'js'],
  ): Promise<void> {
